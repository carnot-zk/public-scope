...
async function buildProgram(program: ProgramDefinition, options: BuildOptions = {}) {
...
export async function runBuildTasks(programs: ProgramDefinition[], options: BuildOptions = {}) {
  if (!programs.length) {
    throw new Error("No programs selected for build.");
  }

  const uniquePrograms = [
    ...new Map(programs.map((p) => [p.anchorName, p])).values(),
  ];
